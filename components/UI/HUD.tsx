...
import { Heart, Zap, Trophy, MapPin, Diamond, Rocket, ArrowUpCircle, Shield, Activity, PlusCircle, Play, Pause, Home } from 'lucide-react';
import { useStore } from '../../store';
import { GameStatus, NEBULA_COLORS, ShopItem, RUN_SPEED_BASE } from '../../types';
...
export const HUD: React.FC = () => {
  const { 
      ...
      restartGame, startGame, gemsCollected, distance, isImmortalityActive, speed,
      togglePause, quitToMenu
  } = useStore();
  
  const target = ['N', 'E', 'B', 'U', 'L', 'A'];
  ...
  if (status === GameStatus.PAUSED) {
      return (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md z-[100] flex flex-col items-center justify-center pointer-events-auto animate-in fade-in duration-200">
              <h1 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-widest font-cyber drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">PAUSED</h1>
              <div className="flex flex-col space-y-4 w-64">
                  <button 
                      onClick={togglePause}
                      className="flex items-center justify-center px-6 py-4 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg transition-all shadow-[0_0_15px_rgba(0,255,255,0.3)]"
                  >
                      <Play className="mr-2 fill-white" /> RESUME
                  </button>
                  <button 
                      onClick={quitToMenu}
                      className="flex items-center justify-center px-6 py-4 bg-gray-700 hover:bg-red-600 text-white font-bold rounded-lg transition-all border border-white/10"
                  >
  ...
  if (status === GameStatus.MENU) {
      return (
          <div className="absolute inset-0 flex items-center justify-center z-[100] bg-black/80 backdrop-blur-sm p-4 pointer-events-auto">
              {/* Card Container */}
              <div className="relative w-full max-w-md rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,255,255,0.2)] border border-white/10 animate-in zoom-in-95 duration-500 bg-gray-900 flex flex-col items-center justify-center p-8">
                
                {/* Title Header */}
                <div className="flex flex-col items-center justify-center mb-8">
                    <h1 className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-purple-500 to-pink-500 font-cyber tracking-wider drop-shadow-[0_0_10px_rgba(0,255,255,0.5)] text-center">
                        NEBULA<br/>RUN
                    </h1>
                </div>
                     
                {/* Content positioned at the bottom of the card */}
                <div className="flex flex-col justify-end items-center w-full z-10">
                    <button 
          ...
          <div className="absolute inset-0 bg-black/90 z-[100] text-white pointer-events-auto backdrop-blur-sm overflow-y-auto">
              <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <div className="flex space-x-4">
                    <button 
                      onClick={() => { quitToMenu(); }}
                      className="px-6 py-3 md:py-4 bg-gray-800 text-white font-bold text-lg rounded hover:bg-gray-700 transition-all border border-white/20"
                    >
                        MENU
                    </button>
                    <button 
            ...
            <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <div className="grid grid-cols-1 gap-4 text-center mb-8 w-full max-w-md">
                 ...
                 <div className="flex space-x-4">
                    <button 
                      onClick={() => { quitToMenu(); }}
                      className="px-6 py-4 md:py-5 bg-gray-800 text-white font-bold text-lg rounded hover:bg-gray-700 transition-all border border-white/20"
                    >
                        MENU
                    </button>
                    <button 
    ...
    <div className={containerClass}>
        ...
        <div className="flex justify-between items-start w-full">
            ...
            <div className="flex items-center gap-4">
                ...
                <button 
                    onClick={togglePause}
                    className="pointer-events-auto p-2 bg-white/10 hover:bg-white/20 rounded-full backdrop-blur text-cyan-400 border border-white/20 transition-colors"
